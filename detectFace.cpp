...
void detect_dnn_face()
{
	...
	while (true)
	{
		...
		{
			float confidence = detect.at<float>(i, 2);	//행렬 2번째 값은 정확도
			if (confidence < 0.7) break;
			int x1 = cvRound(detect.at<float>(i, 3) * frame.cols);
			int y1 = cvRound(detect.at<float>(i, 4) * frame.rows);	//시작포인터
			...
			cv::putText(frame, label, cv::Point(x1, y1 - 1), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(255, 0, 0));
